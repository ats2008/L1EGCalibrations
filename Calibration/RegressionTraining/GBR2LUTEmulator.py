...
class GBR2LUTEmulator:
    ...
    def retrieveForest(self):
        ...
        self.forest = self.inputFile.Get(self.forestName) #, self.forest)
...
#"""
            ...
            for inputs in product(*inputPoints):
                ...
                for var,inp in zip(self.inputVariables,inputs):
                    strInputs += var+"="+f"{inp:.3f}"+" "

                y = corr # (ET_off/ET_L1/0.5)
                if y<0:
                    y = 0
                if y>1.8:
                    y=1.8 
                yint = int(round(y*256)) # [0,2] -> [0, 512] (9 bits)
                #print(y/2,yint, f"[ cor : {corr:.3f} , sc : {scale:.3f} , resp : {resp:.3f}, math.sin(rsp) : {math.sin(resp)} ]")
                if yint>511: # threshold
                    yint=511

                print(address, yint, "#", strInputs, "sf : ",corr," -> ",y,file=output)
                address += 1
